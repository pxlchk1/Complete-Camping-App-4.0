...
export const PACKING_LIBRARY_ITEMS: PackingLibraryItem[] = [
  ...
  createItem('Tent stakes', 'shelter', 4, baseTags(['car_camping', 'backpacking']), {
    defaultQty: 8,
  }),
  ...
  createItem(
    ...
    'shelter',
    3,
    baseTags(['car_camping', 'backpacking']),
  ),
  createItem('Tent repair kit', 'shelter', 2, baseTags(['backpacking'])),

  // Shelter - Winter/Wind specific
  createItem('4-season tent', 'shelter', 5, winterTags(['backpacking', 'car_camping']), {
    notes: 'Required for winter camping',
  }),
  createItem(
    'Snow stakes or deadman anchors',
    'shelter',
    3,
    snowTags(['backpacking', 'car_camping']),
  ),
  ...
  createItem('Cold-weather sleeping bag (0-20°F)', 'sleep_system', 5, winterTags(), {
    notes: 'Critical for winter camping',
  }),
  createItem('Insulated sleeping pad (R4+)', 'sleep_system', 5, winterTags(), {
    notes: 'R-value of 4+ for winter',
  }),
  createItem('Backup foam pad', 'sleep_system', 3, winterTags(['backpacking'])),
  createItem('Sleeping bag liner', 'sleep_system', 3, coldTags(), {
    notes: 'Adds 10-15°F warmth',
  }),
  createItem('Hot water bottle', 'sleep_system', 2, winterTags(), {
    notes: 'Fill with hot water before bed',
  }),
  ...
  createItem('Insulated water bottle sleeve', 'water', 3, winterTags(), {
    notes: 'Prevents freezing',
  }),
  createItem('Wide-mouth water bottles', 'water', 3, winterTags(), {
    notes: 'Less likely to freeze shut',
  }),
  ...
  createItem('Thermos', 'kitchen', 3, winterTags(), {
    notes: 'For hot drinks on the trail',
  }),
  createItem('Windscreen for stove', 'kitchen', 4, windyTags(), {
    notes: 'Essential in windy conditions',
  }),
  createItem('Cold-weather fuel', 'kitchen', 4, winterTags(['backpacking']), {
    notes: 'Isobutane struggles below 20°F',
  }),

  // ============================================================================
  // FOOD - Base items
  // ============================================================================
  createItem('Breakfasts', 'food', 4, baseTags(), {
    notes: 'Plan per number of nights + 1',
  }),
  ...
  createItem('High-calorie snacks', 'food', 4, winterTags(), {
    notes: 'Body burns more calories in cold',
  }),
  createItem('Hot drink packets', 'food', 3, winterTags(), {
    notes: 'Hot cocoa, cider, tea',
  }),
  createItem('Extra food rations', 'food', 3, winterTags(), {
    notes: 'Plan extra for cold weather',
  }),

  // Food - Summer specific
  createItem('Electrolyte powder', 'food', 3, hotTags(), {
  ...
  // ============================================================================
  createItem('Base layer top (wool or synthetic)', 'layers_and_warmth', 5, coldTags(), {
    notes: 'Moisture-wicking, never cotton',
  }),
  ...
  createItem(
    ...
    5,
    winterTags(),
    { notes: 'Your main warmth layer' },
  ),
  createItem('Puffy vest', 'layers_and_warmth', 3, coldTags()),
  createItem('Warm hat / beanie', 'layers_and_warmth', 5, winterTags(), {
    notes: 'Lose 40% of heat through head',
  }),
  createItem('Gloves', 'layers_and_warmth', 4, coldTags()),
  createItem('Liner gloves', 'layers_and_warmth', 3, winterTags(), {
    notes: 'For camp tasks in cold',
  }),
  createItem('Insulated gloves / mittens', 'layers_and_warmth', 4, winterTags(), {
    notes: 'Mittens are warmer than gloves',
  }),
  createItem('Neck gaiter / buff', 'layers_and_warmth', 4, coldTags()),
  createItem('Balaclava', 'layers_and_warmth', 3, winterTags(), {
    notes: 'Full face protection in extreme cold',
  }),
  ...
  createItem('Extra wool socks', 'layers_and_warmth', 4, winterTags(), {
    defaultQty: 2,
    notes: 'Keep dry socks for sleeping',
  }),
  ...
  // Winter footwear
  createItem('Insulated boots', 'footwear', 5, winterTags(), {
    notes: 'Rated for expected temperatures',
  }),
  ...
  // Winter first aid
  createItem('Hand/skin repair cream', 'first_aid', 3, winterTags(), {
    notes: 'For cracked/dry skin',
  }),
  ...
  // Winter electronics
  createItem('Extra power bank', 'electronics', 3, winterTags(), {
    notes: 'Cold drains batteries faster',
  }),

  // ============================================================================
  // CAR CAMPING SPECIFIC
  // ============================================================================
  createItem('Camp chairs', 'trip_specific', 3, baseTags(['car_camping']), {
    defaultQty: 2,
  }),
  ...
  createItem('Bear canister / bag', 'trip_specific', 4, baseTags(['backpacking']), {
    notes: 'Required in many areas',
  }),
  createItem('Permit', 'trip_specific', 5, baseTags(['backpacking']), {
    notes: 'Check if required',
  }),
];
...
export function getSuggestedItems(context: {
  ...
  return PACKING_LIBRARY_ITEMS.filter((item) => {
    ...
    // Check season match
    if (
      item.tags.seasons.includes(context.season as any) ||
      item.tags.seasons.includes('any')
    ) {
