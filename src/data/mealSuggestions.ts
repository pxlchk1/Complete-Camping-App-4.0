...
export const ENHANCED_MEAL_SUGGESTIONS: MealSuggestion[] = [
  ...
  },
  {
    ...
    ingredients: [
      {
        item: 'Chicken breasts or ground beef',
        quantity: 1.5,
        unit: 'lbs',
        category: 'protein',
      },
  ...
  },
  {
    ...
    instructions: ['Wash fruit', 'Cut if needed', 'Serve'],
    popularityScore: 75,
  },
  {
    id: 's4',
    ...
    instructions: ['Arrange crackers and cheese', 'Slice sausage if using', 'Serve'],
    popularityScore: 85,
  },
  {
    id: 's5',
  ...
  },
  {
    ...
    instructions: ['Pour salsa into bowl', 'Serve with chips'],
    popularityScore: 88,
  },
  {
    id: 's7',
    ...
    storage: 'none',
    dietaryTags: [],
    ingredients: [{ item: 'Beef jerky', quantity: 1, unit: 'bag', category: 'snacks' }],
...
export function filterSuggestions(
  ...
  return suggestions.filter((suggestion) => {
    ...
    if (
      ...
      !suggestion.cookingMethods.includes(filters.cookingMethod)
    )
      return false;

    if (filters.maxComplexity) {
      const complexityOrder = { easy: 1, moderate: 2, advanced: 3 };
      if (complexityOrder[suggestion.complexity] > complexityOrder[filters.maxComplexity])
        return false;
    }

    if (filters.dietary && filters.dietary.length > 0) {
      if (!filters.dietary.every((tag) => suggestion.dietaryTags.includes(tag)))
        return false;
    }

    if (
      filters.maxPrepTime &&
      suggestion.prepTime + suggestion.cookTime > filters.maxPrepTime
    )
      return false;

    if (
      filters.storage &&
      suggestion.storage !== filters.storage &&
...
export function getTopSuggestions(
  ...
  return ENHANCED_MEAL_SUGGESTIONS.filter((s) => s.mealType === mealType)
