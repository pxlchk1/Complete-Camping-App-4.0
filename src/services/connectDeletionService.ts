...
export async function canUserDeleteContent(
  ...
  try {
    ...
    console.error(
      `[canUserDeleteContent] Error checking permissions for ${collectionName}/${docId}:`,
      {
        code: error.code,
...
export async function deleteConnectContent(
  ...
  if (!user) {
    ...
    return {
      ...
      error: {
        ...
        message: 'Must be signed in to delete content',
      },
    };
  }
  ...
  try {
    ...
    const { canDelete, isOwner, isAdmin } = await canUserDeleteContent(
      collectionName,
      docId,
      ownerField,
    );
...
export async function deleteQuestion(questionId: string): Promise<DeleteResult> {
  ...
  if (result.success) {
    ...
    try {
      ...
      console.warn(
        `${logPrefix} Could not clean up answers (may already be deleted):`,
        err,
      );
    }
  }
...
export async function deletePhotoPost(photoId: string): Promise<DeleteResult> {
  ...
  if (!user) {
    ...
    return {
      ...
      error: {
        code: 'auth/not-authenticated',
        message: 'Must be signed in to delete photos',
      },
    };
  }
