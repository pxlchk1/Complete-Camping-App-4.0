 ...
 * Uses React Native Firebase Analytics (native SDK)
 */
...
class AnalyticsService {
  ...
  async trackOnboardingStarted(): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.ONBOARDING_STARTED, {
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
  ...
  async trackOnboardingCompleted(reason: OnboardingCompletionReason): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.ONBOARDING_COMPLETED, {
        reason,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
  ...
  async trackPermissionPromptShown(type: PermissionType): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.PERMISSION_PROMPT_SHOWN, {
        type,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
   ...
   */
  async trackPermissionResult(
    type: PermissionType,
    status: PermissionStatus,
  ): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.PERMISSION_RESULT, {
        type,
        status,
        timestamp: new Date().toISOString(),
      });
  ...
  async trackPushSent(key: string): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.PUSH_SENT, {
        key,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
  ...
  async trackPushOpened(key: string): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.PUSH_OPENED, {
        key,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
  ...
  async trackEmailSent(key: string): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.EMAIL_SENT, {
        key,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
  ...
  async trackEmailClicked(key: string): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.EMAIL_CLICKED, {
        key,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
  ...
  async trackTripCreated(tripId?: string): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.TRIP_CREATED, {
        trip_id: tripId,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
  ...
  async trackPackingListGenerated(tripId?: string): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.PACKINGLIST_GENERATED, {
        trip_id: tripId,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
  ...
  async trackGearItemAdded(itemCount?: number): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.GEAR_ITEM_ADDED, {
  ...
  async trackSavedPlaceAdded(placeType?: string): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.SAVED_PLACE_ADDED, {
  ...
  async trackWeatherAddedToTrip(tripId?: string): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.WEATHER_ADDED_TO_TRIP, {
        trip_id: tripId,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
  ...
  async trackBuddyInviteSent(method?: 'email' | 'text' | 'copy'): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.BUDDY_INVITE_SENT, {
        method,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
  ...
  async trackReturnDay7(): Promise<void> {
    try {
      await analytics().logEvent(AnalyticsEvents.RETURN_DAY_7, {
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
...
export const trackOnboardingCompleted = (reason: OnboardingCompletionReason) =>
  analyticsService.trackOnboardingCompleted(reason);
export const trackPermissionPromptShown = (type: PermissionType) =>
  analyticsService.trackPermissionPromptShown(type);
export const trackPermissionResult = (type: PermissionType, status: PermissionStatus) =>
  analyticsService.trackPermissionResult(type, status);
...
export const trackTripCreated = (tripId?: string) =>
  analyticsService.trackTripCreated(tripId);
export const trackPackingListGenerated = (tripId?: string) =>
  analyticsService.trackPackingListGenerated(tripId);
export const trackGearItemAdded = (itemCount?: number) =>
  analyticsService.trackGearItemAdded(itemCount);
export const trackSavedPlaceAdded = (placeType?: string) =>
  analyticsService.trackSavedPlaceAdded(placeType);
export const trackWeatherAddedToTrip = (tripId?: string) =>
  analyticsService.trackWeatherAddedToTrip(tripId);
export const trackBuddyInviteSent = (method?: 'email' | 'text' | 'copy') =>
  analyticsService.trackBuddyInviteSent(method);
...
export const trackPaywallShown = (triggerKey: string, variant?: string) =>
  analyticsService.trackEvent(AnalyticsEvents.PAYWALL_SHOWN, {
    trigger_key: triggerKey,
    variant,
  });
export const trackPaywallCtaTapped = (triggerKey: string, variant?: string) =>
  analyticsService.trackEvent(AnalyticsEvents.PAYWALL_PRIMARY_CTA_TAPPED, {
    trigger_key: triggerKey,
    variant,
  });
export const trackPaywallDismissed = (triggerKey: string, variant?: string) =>
  analyticsService.trackEvent(AnalyticsEvents.PAYWALL_DISMISSED, {
    trigger_key: triggerKey,
    variant,
  });
export const trackAccountRequiredShown = (triggerKey: string) =>
  analyticsService.trackEvent(AnalyticsEvents.ACCOUNT_REQUIRED_SHOWN, {
    trigger_key: triggerKey,
  });
export const trackAccountRequiredCtaTapped = (triggerKey: string) =>
  analyticsService.trackEvent(AnalyticsEvents.ACCOUNT_REQUIRED_CTA_TAPPED, {
    trigger_key: triggerKey,
  });
export const trackAccountRequiredDismissed = (triggerKey: string) =>
  analyticsService.trackEvent(AnalyticsEvents.ACCOUNT_REQUIRED_DISMISSED, {
    trigger_key: triggerKey,
  });
export const trackProAttemptGate = (gateKey: string, attemptCount: number) =>
  analyticsService.trackEvent(AnalyticsEvents.PRO_ATTEMPT_GATE, {
    gate_key: gateKey,
