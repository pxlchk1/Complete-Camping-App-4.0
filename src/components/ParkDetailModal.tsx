...
export default function ParkDetailModal({
  ...
  const handleAddToTripWithConfirmation = useCallback(
    (tripId?: string) => {
      if (isGuest) {
        onClose();
      ...
      <View style={{ flex: 1, backgroundColor: PARCHMENT }}>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20 }}>
          ...
          <View style={{ width: '100%', marginBottom: 24, gap: 12 }}>
            ...
            {tripIdForDestination ? (
              <Animated.View
                style={{ transform: [{ scale: destinationSet ? successScale : 1 }] }}
              >
                <Pressable
              ...
              activeTrip && (
                <Animated.View
                  style={{
                    transform: [
                      { scale: addedToTripId === activeTrip.id ? successScale : 1 },
                    ],
                  }}
                >
                  <Pressable
                    ...
                    style={{
                      backgroundColor:
                        addedToTripId === activeTrip.id ? SUCCESS_GREEN : PARCHMENT,
                      borderRadius: 16,
                      paddingVertical: 10,
                      paddingHorizontal: 20,
                      borderWidth: 1,
                      borderColor:
                        addedToTripId === activeTrip.id ? SUCCESS_GREEN : BORDER_SOFT,
                      flexDirection: 'row',
                      alignItems: 'center',
                      justifyContent: 'center',
