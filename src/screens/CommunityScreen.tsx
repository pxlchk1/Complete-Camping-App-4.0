...
import {
  useGearReviewStore,
  useGearReviews,
  useGearReviewsLoading,
} from '../state/gearReviewStore';
...
export default function CommunityScreen() {
  ...
  const [activeTab, setActiveTab] = useState<
    'tips' | 'connect' | 'images' | 'feedback' | 'gear'
  >(initialTab || 'tips');
    ...
    <View>
      ...
      <View className="bg-forest" style={{ paddingVertical: 12 }}>
        ...
        >
          ...
          {user && (
            <Pressable onPress={handleSubmitTip} className="ml-3 active:opacity-70">
      ...
      <View className="px-4 mt-4">
        {!user ? (
          <View
            className="rounded-xl p-8 items-center border"
          ...
          >
            ...
            >
              <Text
                style={{ fontFamily: 'SourceSans3_600SemiBold', color: TEXT_ON_DARK }}
              >
                Sign in
              </Text>
            </Pressable>
              ...
              {feedbackPosts
                ...
                .map((post) => (
                  <Pressable
                    key={post.id}
                    onPress={() =>
                      navigation.navigate('FeedbackDetail', { postId: post.id })
                    }
                  ...
                  >
                    ...
                    >
                      ...
                      <Text
                        style={{ marginHorizontal: 6, opacity: 0.7, color: TEXT_MUTED }}
                      >
                        •
                      </Text>
                      <Text
              ...
              {gearReviews
                ...
                .map((review) => (
                  <Pressable
                    key={review.id}
                    onPress={() =>
                      navigation.navigate('GearReviewDetail', { reviewId: review.id })
                    }
                    className="rounded-xl p-4 border active:opacity-70"
                    style={{
                      backgroundColor: CARD_BACKGROUND_LIGHT,
