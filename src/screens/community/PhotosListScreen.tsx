...
export default function PhotosListScreen() {
  ...
  const loadPhotoPosts = useCallback(
    async (refresh = false) => {
      try {
        ...
        const visiblePosts = result.posts.filter(
          (post) => !post.isHidden || post.userId === currentUser?.id,
        );
  ...
  const handleVote = async (postId: string, direction: 'up' | 'down') => {
    ...
    setPhotoPosts((prev) =>
      prev.map((p) =>
        p.id === postId ? { ...p, voteCount: (p.voteCount || 0) + delta } : p,
      ),
    );
        ...
        >
          ...
          <Pressable
            ...
            style={{
              ...
              alignItems: 'center',
              backgroundColor:
                tagsExpanded || selectedTag ? DEEP_FOREST + '15' : 'transparent',
              borderWidth: 1,
  ...
  const renderPhotoItem = ({
    ...
    item: PhotoPost | Story;
    index: number;
  }) => {
    const isPhotoPost = 'photoUrls' in item;
    const imageUrl = isPhotoPost
      ? (item as PhotoPost).photoUrls?.[0]
      : (item as Story).imageUrl;
                ...
                {isPhotoPost && (item as PhotoPost).postType && (
                  <View
                    style={{
                      backgroundColor:
                        POST_TYPE_COLORS[
                          mapLegacyPostType((item as PhotoPost).postType)
                        ] + '30',
                      paddingHorizontal: 6,
                      paddingVertical: 2,
                      borderRadius: 8,
                ...
                {isPhotoPost && (
                  ...
                  >
                    ...
                    >
                      <Ionicons
                        name={
                          currentVote === 'down' ? 'arrow-down' : 'arrow-down-outline'
                        }
                        size={14}
