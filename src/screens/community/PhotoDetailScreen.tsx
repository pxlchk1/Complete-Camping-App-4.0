...
export default function PhotoDetailScreen() {
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            if (result.success) {
              ...
              Alert.alert('Error', result.error?.message || 'Failed to remove photo');
            }
          },
        },
      ],
  ...
  const handleCommentVote = async (commentId: string, direction: 'up' | 'down') => {
    ...
    try {
      const result = await voteOnPhotoComment(
        postId,
        commentId,
        currentUser.id,
        direction,
  ...
  const imageUrl = isNewFormat ? photoPost?.photoUrls?.[0] : photo?.imageUrl;
        ...
        >
            ...
            >
              <View className="flex-row items-center flex-1">
                ...
                {rawHandle && authorUserId ? (
                  ...
                  <Pressable
                    onPress={() =>
                      navigation.navigate('MyCampsite', { userId: authorUserId })
                    }
                  >
                    <Text
                      className="text-sm"
              ...
              <ContentActionsAffordance
                ...
                itemType="photo"
                createdByUserId={
                  isNewFormat ? photoPost?.userId || '' : photo?.userId || ''
                }
          ...
          {isNewFormat && comments.length > 0 && (
            <View style={{ marginHorizontal: 20, marginBottom: 16 }}>
              ...
              {comments
                .filter(
                  (comment) => !comment.isHidden || comment.userId === currentUser?.id,
                )
                .map((comment) => (
                  <View
                    key={comment.id}
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <Ionicons
                            name={
                              comment.userVote === 'up' ? 'arrow-up' : 'arrow-up-outline'
                            }
                            size={16}
                            color={
                              comment.userVote === 'up' ? EARTH_GREEN : TEXT_SECONDARY
                            }
                          />
                        </Pressable>
                        <Text
                        ...
                        >
                          <Ionicons
                            ...
                            size={16}
                            color={
                              comment.userVote === 'down' ? '#dc2626' : TEXT_SECONDARY
                            }
                          />
                        </Pressable>
                      </View>
