...
export default function FeedbackDetailScreen() {
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            if (result.success) {
              ...
              Alert.alert('Error', result.error?.message || 'Failed to delete feedback');
            }
          },
        },
      ],
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            if (result.success) {
              ...
              Alert.alert('Error', result.error?.message || 'Failed to remove feedback');
            }
          },
        },
      ],
  ...
  const loadPostData = async () => {
    try {
      ...
      const errorMessage =
        typeof err?.message === 'string' ? err.message : 'Failed to load post';
    ...
    requireProForAction(
      ...
      },
      {
        ...
        openPaywallModal: (variant) =>
          navigation.navigate('Paywall', { triggerKey: 'feedback_comment', variant }),
      },
    );
  };
    ...
    <View className="flex-1 bg-parchment">
      ...
      >
        <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 20 }}>
          {/* Post Card */}
          <View
            className="mx-5 mt-5 rounded-xl p-5 border"
            ...
            <View className="flex-row items-start justify-between mb-3">
              <View className="flex-row flex-wrap gap-2 flex-1">
                ...
                >
                  <Text
                    className="text-xs"
                    style={{ fontFamily: 'SourceSans3_600SemiBold', color: statusColor }}
                  >
                    {statusLabel}
                  </Text>
                </View>
            ...
            >
              <View>
                {post.authorId ? (
                  <Pressable
                    onPress={() =>
                      navigation.navigate('MyCampsite', { userId: post.authorId })
                    }
                  >
                    <Text
                      className="text-xs"
          ...
          <View className="mx-5 mt-6 mb-5">
            ...
            >
              <TextInput
                ...
                placeholder={
                  currentUser ? 'Share your thoughts...' : 'Upgrade to Pro to comment...'
                }
              ...
              />
              <View
                className="flex-row justify-end p-3 border-t"
                style={{ borderColor: BORDER_SOFT }}
              >
                <Pressable
                  ...
                  style={{
                    backgroundColor:
                      commentText.trim() && !submitting ? DEEP_FOREST : '#d1d5db',
                  }}
                >
                  {submitting ? (
