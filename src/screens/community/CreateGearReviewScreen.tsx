...
export default function CreateGearReviewScreen() {
  ...
  const normalizeTag = (raw: string) =>
    raw.trim().replace(/\s+/g, ' ').replace(/^[#]+/, '').slice(0, 24);
  ...
  const onSubmit = useCallback(async () => {
    ...
    if (!trimmedGearName || !trimmedSummary || !trimmedBody || rating <= 0) {
      Alert.alert(
        'Missing info',
        'Please fill out all required fields and select a rating.',
      );
      return;
    }
              ...
              return (
                ...
                >
                  <Text
                    style={{
                      fontFamily: 'SourceSans3_600SemiBold',
                      color: active ? PARCHMENT : TEXT_PRIMARY_STRONG,
                    }}
                  >
                    {c.label}
                  </Text>
            ...
            {[1, 2, 3, 4, 5].map((i) => (
              <Pressable key={i} onPress={() => setStarRating(i)} hitSlop={8}>
                <Ionicons
                  ...
                  size={30}
                  color={rating >= i ? '#d4a017' : TEXT_MUTED}
                />
              </Pressable>
            ))}
            <Text
