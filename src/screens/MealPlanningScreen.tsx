...
export default function MealPlanningScreen() {
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<SuggestibleMealCategory>('breakfast');
  ...
  const [mealSheetInitialTab, setMealSheetInitialTab] = useState<
    'suggest' | 'recipes' | 'custom'
  >('suggest');
  ...
  const { showModal, currentTooltip, dismissModal, openModal } =
    useScreenOnboarding('MealPlanning');
    ...
    if (
      !requirePro({
        ...
        openPaywallModal: (variant) =>
          navigation.navigate('Paywall', { triggerKey: 'meal_planning_add', variant }),
      })
    ) {
      return;
    }
  ...
  const handleAddCustomMeal = async () => {
    ...
    if (
      !requirePro({
        openAccountModal: () => setShowAccountModal(true),
        openPaywallModal: (variant) =>
          navigation.navigate('Paywall', { triggerKey: 'meal_planning_custom', variant }),
      })
    ) {
      return;
    }
  ...
  const handleDeleteMeal = async (meal: Meal) => {
    ...
    if (
      !requirePro({
        openAccountModal: () => setShowAccountModal(true),
        openPaywallModal: (variant) =>
          navigation.navigate('Paywall', { triggerKey: 'meal_planning_delete', variant }),
      })
    ) {
      return;
    }
  ...
  const handleOpenMealSheet = (
    category: SuggestibleMealCategory,
    tab: 'suggest' | 'recipes' | 'custom' = 'suggest',
  ) => {
    ...
    if (tab === 'suggest') {
      if (
        !requirePro({
          openAccountModal: () => setShowAccountModal(true),
          openPaywallModal: (variant) =>
            navigation.navigate('Paywall', { triggerKey: 'meal_suggestions', variant }),
        })
      ) {
        return;
      }
  ...
  const handleAddSuggestionFromPicker = async (suggestion: MealSuggestion) => {
    try {
      ...
      showToast(
        `Added to ${MEAL_CATEGORIES.find((c) => c.key === suggestion.category)?.label || 'meal'}`,
      );
    } catch (error) {
  ...
  const handleReplaceSuggestionFromPicker = async (suggestion: MealSuggestion) => {
    try {
      ...
      setShowSuggestionPicker(false);
      showToast(
        `Replaced ${MEAL_CATEGORIES.find((c) => c.key === suggestion.category)?.label || 'meal'}`,
      );
  ...
  const addMealFromSuggestion = async (suggestion: MealSuggestion) => {
    try {
      const mealData = {
        ...
        dayIndex: selectedDay,
        sourceType:
          suggestion.type === 'recipe' ? ('library' as const) : ('custom' as const),
  ...
  const handleSelectRecipe = async (recipe: MealLibraryItem) => {
    ...
    if (
      !requirePro({
        openAccountModal: () => setShowAccountModal(true),
        openPaywallModal: (variant) =>
          navigation.navigate('Paywall', { triggerKey: 'meal_planning_recipe', variant }),
      })
    ) {
      return;
    }
  ...
  const handleSelectSuggestion = async (suggestion: MealSuggestion) => {
    ...
    if (
      !requirePro({
        openAccountModal: () => setShowAccountModal(true),
        openPaywallModal: (variant) =>
          navigation.navigate('Paywall', { triggerKey: 'meal_suggestion', variant }),
      })
    ) {
      return;
    }
  ...
  const handleSheetCustomMeal = async (
    name: string,
    ingredients?: string[],
    notes?: string,
  ...
  const handleOpenAutoFillPreview = () => {
    ...
    if (
      !requirePro({
        openAccountModal: () => setShowAccountModal(true),
        openPaywallModal: (variant) =>
          navigation.navigate('Paywall', { triggerKey: 'meal_autofill', variant }),
      })
    ) {
      return;
    }
  ...
  const handleAddRecipeToMeal = async (recipe: MealLibraryItem) => {
    ...
    try {
      ...
      showToast(
        `Added to ${MEAL_CATEGORIES.find((c) => c.key === targetCategory)?.label || 'meal'}`,
      );
    } catch (error) {
    ...
    <SafeAreaView className="flex-1 bg-parchment" edges={['top']}>
      ...
      <View className="px-5 pt-4 pb-3 border-b border-stone-200">
        ...
        <View className="mt-3 flex-row items-center justify-between">
          ...
          <View
            className="flex-row rounded-lg overflow-hidden border"
                      ...
                      <View className="bg-white rounded-xl p-4 mb-2 border border-stone-200">
                        ...
                        >
                          <Pressable
                            onPress={() =>
                              handleOpenMealSheet(
                                category.key as SuggestibleMealCategory,
                                'suggest',
                              )
                            }
                      ...
                      <>
                        {categoryMeals.map((meal) => (
                          ...
                          >
                            ...
                            <View
                              className="flex-row mt-3 pt-3 border-t flex-wrap"
                              style={{ borderColor: BORDER_SOFT, gap: 8 }}
                            >
                              <Pressable
                                onPress={() =>
                                  handleOpenMealSheet(
                        ...
                        {(category.key === 'snack' || categoryMeals.length === 0) && (
                          <Pressable
                            onPress={() =>
                              handleOpenMealSheet(category.key as SuggestibleMealCategory)
                            }
        ...
        <View className="flex-1">
          ...
          <View className="px-5 py-3 border-b" style={{ borderColor: BORDER_SOFT }}>
            {activeMealContext && (
              ...
              >
                Adding to:{' '}
                {MEAL_CATEGORIES.find((c) => c.key === activeMealContext)?.label}
              </Text>
            )}
            ...
            >
              <Pressable
                ...
                style={{
                  backgroundColor:
                    recipeFilter === 'all' ? DEEP_FOREST : 'rgba(26, 76, 57, 0.1)',
                }}
              >
                <Text
                  style={{
              ...
              {MEAL_CATEGORIES.map((cat) => (
                <Pressable
                  ...
                  style={{
                    backgroundColor:
                      recipeFilter === cat.key ? DEEP_FOREST : 'rgba(26, 76, 57, 0.1)',
                  }}
                >
                  <Text
                    style={{
                        ...
                        {meal.tags && meal.tags.length > 0 && (
                          <View className="flex-row flex-wrap gap-1 mt-2">
                            {meal.tags.slice(0, 3).map((tag, index) => (
                              <View
                                key={index}
                                className="px-2 py-0.5 rounded bg-stone-100"
                              >
                                <Text
                                  className="text-xs"
                                  style={{
      ...
      <SuggestionPickerSheet
        ...
        existingEntryCount={
          dayMeals.filter((m) => m.category === selectedCategory).length
        }
