...
export default function ShoppingListScreen() {
  ...
  useEffect(() => {
    ...
    const ingredientMap = new Map<
      string,
      { mealNames: Set<string>; category?: IngredientCategory }
    >();
    ...
    const categorizeIngredient = (name: string): IngredientCategory => {
      ...
      // Protein
      if (
        /beef|chicken|pork|fish|turkey|sausage|bacon|ham|eggs?|meat|jerky|salmon|shrimp|hot dog/i.test(
          lower,
        )
      ) {
        return 'protein';
      ...
      // Produce
      if (
        /lettuce|tomato|onion|pepper|carrot|potato|apple|orange|banana|berries|fruit|vegetable|zucchini|cucumber|grape|spinach/i.test(
          lower,
        )
      ) {
        return 'produce';
      ...
      // Grains
      if (
        /bread|bun|roll|pasta|rice|tortilla|oatmeal|cereal|pancake|granola|cracker|couscous/i.test(
          lower,
        )
      ) {
        return 'grains';
      ...
      // Condiments
      if (
        /ketchup|mustard|mayo|salsa|dressing|syrup|honey|jam|oil|hummus|peanut butter|guacamole|relish/i.test(
          lower,
        )
      ) {
        return 'condiments';
      ...
      // Spices
      if (
        /salt|pepper|seasoning|spice|garlic|cinnamon|chili|taco|fajita|cumin/i.test(lower)
      ) {
        return 'spices';
      }
      // Beverages
          ...
          <View className="flex-row items-center justify-between mb-3">
            ...
            >
              <Pressable
                ...
                style={{
                  backgroundColor: viewMode === 'category' ? DEEP_FOREST : 'white',
                }}
              >
                <Text
                  style={{
              ...
              {showStaples && suggestedStaples.length > 0 && (
                <View className="mb-4">
                  ...
                  <Text
                    ...
                    style={{ fontFamily: 'SourceSans3_400Regular', color: EARTH_GREEN }}
                  >
                    Common items for a {tripDays}-day trip with {numCampers}{' '}
                    {numCampers === 1 ? 'person' : 'people'}
                  </Text>
