...
import ParkFilterBar, {
  FilterMode,
  ParkType,
  DriveTime,
  SortOption,
  US_STATES,
} from '../components/ParkFilterBar';
...
export default function ParksBrowseScreen({
  onTabChange,
  selectedParkId: selectedParkIdProp,
  onParkDetailClosed,
}: ParksBrowseScreenProps) {
  ...
  const [newCampgroundCoords, setNewCampgroundCoords] = useState<{
    lat: number;
    lng: number;
  } | null>(null);
  ...
  const [accountModalTriggerKey, setAccountModalTriggerKey] = useState<
    string | undefined
  >(undefined);
   ...
   */
  const getParkTypeDisplay = useCallback(
    (filter: string | undefined): TripDestination['parkType'] => {
      if (!filter) return 'Other';
      const normalized = filter.toLowerCase().replace(/_/g, ' ');
      if (normalized.includes('state') && normalized.includes('park'))
        return 'State Park';
      if (normalized.includes('national') && normalized.includes('park'))
        return 'National Park';
      if (normalized.includes('national') && normalized.includes('forest'))
   ...
   */
  const createTripDestinationFromPark = useCallback(
    (park: Park): TripDestination => {
      return {
        sourceType: 'parks',
        placeId: park.id,
        name: park.name,
  ...
  const fetchParks = useCallback(async () => {
    ...
    try {
      ...
      const normalizeParkType = (
        rawFilter: string | undefined | null,
      ): 'national_park' | 'state_park' | 'national_forest' => {
        ...
        if (normalized.includes('state') && normalized.includes('park'))
          return 'state_park';
        if (normalized === 'state_park' || normalized === 'statepark')
          return 'state_park';

        if (normalized.includes('national') && normalized.includes('park'))
          return 'national_park';
        if (normalized === 'national_park' || normalized === 'nationalpark')
          return 'national_park';

        if (normalized.includes('national') && normalized.includes('forest'))
          return 'national_forest';
        if (normalized === 'national_forest' || normalized === 'nationalforest')
          return 'national_forest';
        ...
        if (__DEV__) {
          console.warn(
            `[FILTER_DEBUG] ⚠️ Unknown filter value: "${rawFilter}" -> defaulting to national_forest`,
          );
        }
        return 'national_forest';
      };
      ...
      if (mode === 'state' && selectedState) {
        ...
        fetched = fetched.filter((p) => {
          ...
          return (
            parkState === selectedState.toUpperCase() ||
            parkState === fullStateName.toUpperCase()
          );
        });
        if (__DEV__) {
          console.log(
            `[FILTER_DEBUG] State filter "${selectedState}" (${fullStateName}): ${beforeStateFilter} -> ${fetched.length} parks`,
          );
        }
      }
      ...
      if (parkType !== ('all' as any)) {
        ...
        if (__DEV__) {
          console.log(
            `[FILTER_DEBUG] Type filter "${parkType}": ${beforeTypeFilter} -> ${fetched.length} parks`,
          );
          if (fetched.length === 0) {
            console.warn(
              `[FILTER_DEBUG] ⚠️ No parks match filter "${parkType}"! Check data values.`,
            );
          }
        }
      }
      ...
      if (mode === 'distance' && userLocation) {
        ...
        if (__DEV__) {
          console.log(
            `[FILTER_DEBUG] Distance filter (${maxDistanceMiles} mi): ${beforeDistanceFilter} -> ${fetched.length} parks`,
          );
          console.log(
            `[FILTER_DEBUG] User location: lat=${userLocation.latitude.toFixed(4)}, lng=${userLocation.longitude.toFixed(4)}`,
          );
          if (fetched.length > 0 && fetched.length <= 5) {
            console.log('[FILTER_DEBUG] Parks in range:');
            fetched.forEach((p) =>
              console.log(`  - ${p.name} (${p.filter}): ${p.distance?.toFixed(1)} mi`),
            );
          }
        }
    ...
    if (
      !requirePro({
        ...
        openPaywallModal: (variant) =>
          navigation.navigate('Paywall', { triggerKey: 'custom_campsite', variant }),
      })
    ) {
      return;
    }
  ...
  const handleSaveCampground = async () => {
    ...
    if (!newCampgroundAddress.trim()) {
      Alert.alert(
        'Missing address',
        'Full address is required for weather and navigation.',
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (geocodeResults && geocodeResults.length > 0) {
          ...
          console.log(
            '[ParksBrowse] Geocoded custom campground:',
            geocodedLat,
            geocodedLng,
          );
        } else {
          console.warn(
            '[ParksBrowse] Could not geocode address, continuing without coordinates',
          );
        }
      } catch (geocodeErr) {
      ...
      setNewCampgroundCoords(
        geocodedLat && geocodedLng ? { lat: geocodedLat, lng: geocodedLng } : null,
      );
  ...
  const handleConfirmAddToTrip = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Added!',
        `"${tripDestination.name}" is now the destination for "${trip.name}"`,
      );
    } catch (err) {
    ...
    <View style={styles.root}>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            {trips.length === 0 ? (
              <Text style={{ marginBottom: 20, color: '#111' }}>
                ...
                {trips.map((trip: any) => (
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      backgroundColor:
                        selectedTripId === trip.id ? DEEP_FOREST : CARD_BACKGROUND_LIGHT,
                      marginBottom: 8,
                      borderWidth: 1,
              ...
              >
                <Text
                  style={{
                    ...
                    fontFamily: fonts.bodySemibold,
                    fontSize: 14,
                    opacity: isSavingCampground ? 0.5 : 1,
                  }}
                >
                  Cancel
                </Text>
      ...
      <View style={{ flex: 1, backgroundColor: '#F4EBD0' }}>
        ...
        >
          ...
          >
            <TouchableOpacity onPress={handleAddCampground}>
              <Text
                style={{
                  fontFamily: fonts.bodySemibold,
                  fontSize: fontSizes.sm,
        ...
        <ParkDetailModal
          ...
          onSetAsDestination={async (park, tripId) => {
            ...
            console.log(
              '[ParksBrowse] Setting destination for trip:',
              tripId,
              'Park:',
              park.name,
            );
          ...
          onAddToTrip={(park, tripId) => {
            if (tripId) {
              ...
              console.log(
                '[ParksBrowse] Setting tripDestination for trip:',
                tripId,
                'Park:',
                park.name,
              );
              ...
              const canProceed = requirePro({
                openAccountModal: () => setShowAccountModal(true),
                openPaywallModal: (variant) =>
                  navigation.navigate('Paywall', { triggerKey: 'second_trip', variant }),
              });
              if (!canProceed) {
                setSelectedPark(null);
                onParkDetailClosed?.();
