    ...
    try {
      ...
      for (const { type, collection: collName } of CONTENT_COLLECTIONS) {
        try {
          ...
          const q = query(collRef, where('needsReview', '==', true), limit(20));
          ...
          console.log(
            `[AdminContent] Skipping ${collName}:`,
            collError?.message?.slice(0, 50),
          );
        }
      }
      ...
      try {
        ...
        console.log(
          `[AdminContent] Error querying photo comments:`,
          photoPostsError?.message?.slice(0, 50),
        );
      }
        ...
        <View className="px-5 pt-5 pb-8">
          ...
          {flaggedContent.length === 0 ? (
            <View className="items-center justify-center py-12">
              ...
              >
                There is no content needing moderation at this time. Content with 3+
                downvotes or reports will appear here.
              </Text>
            </View>
          ) : (
            <>
              ...
              >
                {flaggedContent.length} item{flaggedContent.length !== 1 ? 's' : ''} need
                {flaggedContent.length === 1 ? 's' : ''} review
              </Text>
          ...
          >
            <View className="flex-row items-center mb-4">
              <View
                ...
                style={{
                  backgroundColor:
                    confirmModal.action === 'approve' ? '#E8F5E9' : '#FFEBEE',
                }}
              >
                <Ionicons
                  name={
                    confirmModal.action === 'approve' ? 'checkmark-circle' : 'warning'
                  }
                  size={22}
            ...
            <View className="flex-row gap-3">
              ...
              <Pressable
                ...
                style={{
                  backgroundColor:
                    confirmModal.action === 'approve' ? EARTH_GREEN : '#D32F2F',
                }}
              >
                <Text
                  style={{
