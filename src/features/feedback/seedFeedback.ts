...
export async function seedFeedbackIfEmpty() {
  ...
  console.log(
    '[FeedbackSeed] Auth user:',
    auth.currentUser?.email || auth.currentUser?.uid || 'NOT SIGNED IN',
  );
  ...
  try {
    ...
    FEEDBACK_SEED_ITEMS.forEach((item, index) => {
      ...
      console.log(
        `[FeedbackSeed] Preparing doc ${index + 1}/${FEEDBACK_SEED_ITEMS.length}: ${item.title}`,
      );
    ...
    console.log(
      `[FeedbackSeed] ✨ Seeded ${FEEDBACK_SEED_ITEMS.length} feedback posts with karmaScore: 1`,
    );
  } catch (error) {
