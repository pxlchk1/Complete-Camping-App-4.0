...
export const GATING_REGISTRY: GateDefinition[] = [
  ...
  },
  {
    ...
    notes:
      'Guests need account to view trips shared by Campground friends (read-only access)',
  },
  {
...
export function getPaywallTriggerKeys(): string[] {
  return [
    ...new Set(
      GATING_REGISTRY.filter((g) => g.paywallTriggerKey).map((g) => g.paywallTriggerKey!),
    ),
  ];
}
...
export function getAccountModalTriggerKeys(): string[] {
  return [
    ...new Set(
      GATING_REGISTRY.filter((g) => g.accountModalTriggerKey).map(
        (g) => g.accountModalTriggerKey!,
      ),
    ),
  ];
}
...
export function getGatingSummary(): {
  ...
  return {
    ...
    countsTowardProAttempt: GATING_REGISTRY.filter((g) => g.countsTowardProAttempt)
      .length,
