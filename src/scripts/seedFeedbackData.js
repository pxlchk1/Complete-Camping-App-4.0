...
async function seedFeedbackData() {
  ...
  try {
    ...
    console.log(
      `   - Feature Requests: ${SEED_FEEDBACK.filter((f) => f.filterLabel === 'feature').length}`,
    );
    console.log(
      `   - Bug Reports: ${SEED_FEEDBACK.filter((f) => f.filterLabel === 'bug').length}`,
    );
    console.log(
      `   - Improvements: ${SEED_FEEDBACK.filter((f) => f.filterLabel === 'improvement').length}`,
    );
    console.log(
      `   - Questions: ${SEED_FEEDBACK.filter((f) => f.filterLabel === 'question').length}`,
    );
    console.log(
      `   - Other: ${SEED_FEEDBACK.filter((f) => f.filterLabel === 'other').length}`,
    );
  } catch (error) {
