...
export async function updateMembershipTiers() {
  ...
  try {
    ...
    console.log(
      `[UpdateMembershipTiers] Found ${profilesSnapshot.size} profiles to check`,
    );
    ...
    for (const profileDoc of profilesSnapshot.docs) {
      ...
      try {
        ...
        console.log(
          `[UpdateMembershipTiers] Updated ${profileDoc.id}: ${currentTier} → ${newTier}`,
        );
        updatedCount++;
      } catch (error) {
...
export async function seedSampleMembershipTiers() {
  ...
  console.log(
    "4. Edit the 'membershipTier' field to one of: isAdmin, isModerator, subscribed, freeMember",
  );
}
